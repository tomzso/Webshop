...
namespace api.Controllers
{
    ...
    public class OrderItemController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromRoute] int orderId, CreateOrderItemDto orderItemDto)
        {
            ...
            if (product.StockQuantity < orderItemDto.Quantity) return BadRequest("The quantity of one of the order items exceeds the available stock.");
        ...
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] UpdateOrderItemDto orderItemDto)
        {
            ...
            if (previousOrderItem == null) return NotFound("OrderItem id does not exist");
            if (order == null) return BadRequest("Order not exist");
            if (product == null) return BadRequest("Product not exist");
